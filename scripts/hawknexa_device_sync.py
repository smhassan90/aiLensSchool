...
def sync_users(cfg: dict[str, Any]) -> None:
    ...
    if removed:
        log(f"Synced {len(users)} user(s) from terminal — removed {removed} stale in HawkNexa")
    else:
        log(f"Synced {len(users)} user(s) from terminal")
...
def main() -> None:
    ...
    if full_sync_pending:
        log("Full sync from portal — clearing local attendance dedup cache (users sync every cycle)")
        state["fingerprints"] = []
        save_state(state)
