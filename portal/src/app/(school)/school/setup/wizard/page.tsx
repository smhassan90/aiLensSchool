...
export default function SetupWizardPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <PageHeader
        title="Quick start"
        description="Four answers. We create classes, subjects, exam pattern and quiz targets for you."
        actions={
      ...
      <Card className="max-w-xl">
        <CardContent className="p-6">
          <form onSubmit={onSubmit} className="space-y-4">
            <div>
              <Label>This year name</Label>
              <Input name="yearName" defaultValue="2026-27" required />
            </div>
            ...
            <div>
              <Label>How many classes? (1 to 10)</Label>
              <Input name="gradeCount" type="number" min={1} max={12} defaultValue={5} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input name="feeAmount" type="number" min={0} placeholder="5000" />
            </div>
            <div>
              <Label>Minimum quizzes per subject</Label>
              <Input name="minQuizzes" type="number" min={1} defaultValue={4} />
            </div>
            <Button type="submit" disabled={run.isPending}>
              {run.isPending ? "Setting up…" : "Create my school"}
            </Button>
          </form>
        </CardContent>
      </Card>
